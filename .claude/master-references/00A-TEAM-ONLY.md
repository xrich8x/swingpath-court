# The team -- complete, self-contained

**Everything about the five-agent team: why subagents rather than an agent team, the shared
prompt skeleton, all five roles in full, how the lead dispatches, and the journal and memory
layers that let an agent survive being killed.** Nothing about the concurrency cap.

This is an EXTRACT of `00-THE-FULL-SETUP.md` sections 1-2 and 7-12, with the doorman removed
-- that is a separate feature with its own document, `00B-DOORMAN-ONLY.md`. Section numbers
are kept from the source so cross-references still resolve. The team works without the cap;
it simply runs uncapped.

Extracted 2026-09-02. Source of truth for the prompts: `.claude/agents/*.md`.
...
Roles and memory are standard Claude Code features. **The journal is the piece nobody tells
you to build**, and it is what makes a team survive contact with a usage limit.

> A fourth layer, the **doorman**, caps how many agents may run at once. It is a separate
> feature with its own document -- `00B-DOORMAN-ONLY.md` -- and is deliberately not covered
> here. Nothing below depends on it: a team runs fine without one, it just runs uncapped.
...
Symptom: your "spawn 3 agents and synthesise" flow returns nothing; agents appear in
...
whole run — we did this once and lost the run to it. Include `Agent`
...
You may call another teammate directly. Announce the teammate by name and label its output
as theirs, never as your own. A one-word agent still costs **~38k tokens** -- measured, not
estimated -- so call one only when the answer is genuinely outside what you can establish
yourself. If a call is refused because too many agents are already running, your task was
not lost: do not retry it, and do not shrink it to fit.
```
...
**What it does not own:** the investigation (the lead runs it and interrogates
...
> **A surprising RESULT goes to `pm`** (founder ruling
> 2026-08-29): an unexplained gate failure, a number that moved unexpectedly, a claim that
> turns out wrong. Establish what is true and why first; only then does pm
...
(`autoContinueAtUsageLimit: true`). **Nothing restarts the agent, and nothing announces that
it died** -- the failure notice is the only signal, and whatever the agent wrote down is the
whole of what survives. So the first move after any death is to read, not to re-dispatch.

1. **Read the journals.** `.claude/journals/lead.md` first, then the teammate's own.
2. **Check what is actually running** with `ListAgents` before dispatching anything.
3. **Then resume**, preferring the killed agent's uncommitted files over a restart from zero.

If you also run a concurrency cap, this reconciliation has a second half, because a killed
agent never releases its slot. See `00B-DOORMAN-ONLY.md`.

> **A killed agent leaves no notice of its own death.** Whatever it wrote down is the whole
> of what survives -- which is why the journal has to be written during the work, not after.
...
Six hooks run in this repo. Five are listed here because they constrain what an agent can
get away with; the sixth is the concurrency cap, covered in `00B-DOORMAN-ONLY.md`.
...
2. **May agents call each other?** If yes, note that a nested call is invisible to the lead
   -- it cannot see what its children spawn. That is what makes an enforced cap necessary
   rather than optional (`00B-DOORMAN-ONLY.md`).
...
3. **A concurrency cap**, if you want one -- a separate build, see `00B-DOORMAN-ONLY.md`.

### What to change per project

- Replace every domain rule in the role files. The tennis content below runs long; the
  *skeleton* is what transfers, not the geometry.
- Keep the structural features: narrowest tools, the anti-goal written out, the write
  allowlist, the charter guard on delegation, the journal pointer, an explicit output shape.
...
- **A `cd` can leak into a subagent's working directory.** One qa run started in
  `master references/`, not the repo root, and its first memory read 404'd. Anything that
  resolves paths on an agent's behalf should derive its own root rather than trust the cwd.
...
One lead session dispatches four specialist subagents -- pm, backend-dev,
frontend-dev, qa -- each with the narrowest tool set that does its job, its own durable
memory, its own working journal, and a written allowlist of what it may change. Any of them
may call any other. The roles are built around anti-goals as much as goals: pm never
writes code, qa never fixes what it finds, and each carries a guard so that calling a
teammate cannot route around its own charter. Because a usage limit kills an agent outright
and nothing restarts it, every agent writes its journal *during* the work and reads it first
on starting -- a populated journal means resume, not begin. What transfers to another project
is the skeleton, not the tennis: narrowest tools, the anti-goal written out, the write
allowlist, the delegation guard, the journal pointer, and an explicit output shape.
